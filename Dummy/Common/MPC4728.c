...
#include "GI2C1.h"
#include "UTIL1.h"
#define PL_HAS_MPC4728_RDY   1
#define PL_HAS_MPC4728_LDAC  1
#if PL_HAS_MPC4728_RDY
  #include "MPC4728_RDY.h"
#endif
#if PL_HAS_MPC4728_LDAC
...
#if PL_HAS_MPC4728_RDY
  return MPC4728_RDY_GetVal()==0; /* HIGH pin value means ready, LOW means busy */
#else
  return FALSE;
#endif
}
...
static uint8_t PrintStatus(const CLS1_StdIOType *io) {
  ...
  UTIL1_strcpy(buf, sizeof(buf), (unsigned char*)"0x");
  UTIL1_strcatNum8Hex(buf, sizeof(buf), MPC4728_I2C_ADDRESS);
...
#if PL_HAS_MPC4728_RDY
  CLS1_SendStatusStr((unsigned char*)"  RDY Pin", MPC4728_RDY_GetVal()!=0?(unsigned char*)"input: HIGH, ready\r\n":(unsigned char*)"input: LOW, busy\r\n", io->stdOut);
#endif
#if PL_HAS_MPC4728_LDAC
  CLS1_SendStatusStr((unsigned char*)"  LDAC Pin", MPC4728_LDAC_GetVal()!=0?(unsigned char*)"output: HIGH, output with UDAC bit\r\n":(unsigned char*)"output: LOW, immediate\r\n", io->stdOut);
#endif
  return ERR_OK;
}
...
byte MPC4728_ParseCommand(const unsigned char *cmd, bool *handled, const CLS1_StdIOType *io) {
  ...
  if (UTIL1_strcmp((char*)cmd, CLS1_CMD_HELP)==0 || UTIL1_strcmp((char*)cmd, "MPC4728 help")==0) {
  ...
  } else if (UTIL1_strncmp((char*)cmd, "MPC4728 fastwrite all ", sizeof("MPC4728 fastwrite all ")-1)==0) {
    uint16_t dac[4];
    uint8_t pd[4];
    int i;
    
    *handled = TRUE;
    p = cmd+sizeof("MPC4728 fastwrite all ")-1;
    for(i=0;i<4;i++) { /* init */
      dac[i] = 0;
      pd[i] = 0;
    }
    for(i=0;i<4;i++) {
      ...
      dac[i] &= MPC4728_MAX_DAC_VAL; /* ensure it is 12 bits */
    }
    if (i!=4) {
      CLS1_SendStr((unsigned char*)"**** Not enough values, 4 expected!\r\n", io->stdErr);
      return ERR_FAILED;
    }
    if (MPC4728_FastWriteAllDAC(dac, sizeof(dac), pd, sizeof(pd))!=ERR_OK) {
    ...
    *handled = TRUE;
    ...
    if (UTIL1_ScanHex16uNumber(&p, &dac)!=ERR_OK || dac>MPC4728_MAX_DAC_VAL) {
      CLS1_SendStr((unsigned char*)"**** error parsing 12bit hex value, must start with 0x!\r\n", io->stdErr);
      return ERR_FAILED;
    }
    dac &= MPC4728_MAX_DAC_VAL; /* ensure it is 12 bits */
    if (MPC4728_FastWriteDAC(channel, dac)!=ERR_OK) {
    ...
    *handled = TRUE;
    ...
    if (UTIL1_ScanDecimal8uNumber(&p, &ch)!=ERR_OK || ch>3) {
      ...
      if (UTIL1_ScanHex16uNumber(&p, &dac)!=ERR_OK) {
        ...
        if (MPC4728_WriteDACandEE(ch, dac)!=ERR_OK) {
